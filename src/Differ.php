...
function genDiff($pathToFile1, $pathToFile2): string
{
    ...
    $file1 = modifyArray(json_decode(file_get_contents($pathToFile1), true));
    $file2 = modifyArray(json_decode(file_get_contents($pathToFile2), true));

    $keys = array_keys([...$file1, ...$file2]);
    sort($keys);
    ...
    $result = array_reduce($keys,
        function ($acc, $key) use ($file1, $file2) {
            ...
            if (($existsKeyInFile1) && ($existsKeyInFile2)) {
                ...
                if ($existsKeyInFile1) $acc[] = " - $key: $file1[$key]";
                if ($existsKeyInFile2) $acc[] = " + $key: $file2[$key]";
            }
